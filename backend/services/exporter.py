"""Export service — generates PDF (WeasyPrint), DOCX (python-docx), and LaTeX from final CV data."""
...
async def generate_pdf(db: AsyncSession, cv_version: CvVersion) -> bytes:
    """Generate a PDF from a CvVersion using xhtml2pdf (works on Windows)."""
...
async def generate_docx(db: AsyncSession, cv_version: CvVersion) -> bytes:
    """Generate a DOCX from a CvVersion using python-docx (Jake Ryan format)."""
    ...
    # Letter paper margins matching Jake's template
    ...
    def _date_range_docx(start: str, end: str) -> str:
        if start and end:
            return f"{start} \u2013 {end}"
    ...
    def _add_two_row_entry(bold_left, right_top, italic_left, right_bottom):
        """Add a Jake-style two-row entry (bold title + date, italic subtitle + location)."""
        # Row 1
        tbl = doc.add_table(rows=2, cols=2)
        tbl.autofit = True
        tbl.columns[1].width = Inches(2.0)

        # Row 1: bold left | right top
        c00 = tbl.cell(0, 0)
        c00.text = ""
        p = c00.paragraphs[0]
        p.space_after = Pt(0)
        p.space_before = Pt(0)
        run = p.add_run(bold_left)
        run.bold = True
        run.font.size = Pt(10)
        run.font.name = "Times New Roman"
        ...
        p.space_after = Pt(0)
        p.space_before = Pt(0)
        run = p.add_run(right_top)
        run.font.size = Pt(10)
        run.font.name = "Times New Roman"

        # Row 2: italic left | italic right bottom
        c10 = tbl.cell(1, 0)
        c10.text = ""
        p = c10.paragraphs[0]
        p.space_after = Pt(0)
        p.space_before = Pt(0)
        run = p.add_run(italic_left)
        run.italic = True
        run.font.size = Pt(10)
        run.font.name = "Times New Roman"
    ...
    def _add_project_heading(name, tech_stack, dates):
        """Add a single-row project heading (bold name | italic tech | date)."""
    ...
    def _add_bullets(bullets):
        for bullet in bullets:
            bp = doc.add_paragraph(style="List Bullet")
            bp.paragraph_format.space_after = Pt(0)
            bp.paragraph_format.space_before = Pt(0)
            bp.paragraph_format.left_indent = Inches(0.25)
            run = bp.add_run(str(bullet))
            run.font.size = Pt(10)
            run.font.name = "Times New Roman"
    ...
    run = name_para.add_run(profile.get("name") or "Your Name")
    run.bold = True
    run.font.size = Pt(22)
    run.font.name = "Times New Roman"
    ...
    if profile.get("linkedin_url"):
        contact_parts.append(profile["linkedin_url"].replace("https://", "").replace("http://", ""))
    if profile.get("portfolio_url"):
        contact_parts.append(profile["portfolio_url"].replace("https://", "").replace("http://", ""))
    if contact_parts:
    ...
    if context["education"]:
        ...
        for edu in context["education"]:
            dates = _date_range_docx(edu.get("date_start", ""), edu.get("date_end", ""))
            _add_two_row_entry(
            ...
            for a in edu.get("achievements", []):
                items.append(str(a))
            if edu.get("modules"):
                items.append("Coursework: " + ", ".join(edu["modules"]))
            if items:
                _add_bullets(items)
    ...
    if context["experiences"]:
        ...
        for exp in context["experiences"]:
            dates = _date_range_docx(exp.get("date_start", ""), exp.get("date_end", ""))
            _add_two_row_entry(
                exp.get("role_title", "Role"),
                dates,
                exp.get("company", "Company"),
                exp.get("location", ""),
            )
            if exp.get("bullets"):
                _add_bullets(exp["bullets"])

    # Projects
    if context["projects"]:
        _add_section_heading("Projects")
        for proj in context["projects"]:
            dates = _date_range_docx(proj.get("date_start", ""), proj.get("date_end", ""))
            tech = ", ".join(proj.get("skill_tags", []))
            _add_project_heading(proj.get("name", "Project"), tech, dates)
            if proj.get("bullets"):
                _add_bullets(proj["bullets"])

    # Activities
    if context["activities"]:
        _add_section_heading("Leadership & Activities")
        for act in context["activities"]:
            dates = _date_range_docx(act.get("date_start", ""), act.get("date_end", ""))
            _add_two_row_entry(
                act.get("role_title", "Role"),
                dates,
                act.get("organization", "Organization"),
                act.get("location", ""),
            )
            if act.get("bullets"):
                _add_bullets(act["bullets"])

    # Skills
    if context["skills_by_category"]:
        _add_section_heading("Technical Skills")
        for category, skill_list in context["skills_by_category"].items():
            p = doc.add_paragraph()
